...
- Home Assistant: HASS_TOKEN in BWS - try BWS first for HA auth. Gateway restart needs separate SSH (sudo systemctl restart hermes-gateway.service) - blocked from within gateway. Entity registry ops need WebSocket API (pip websocket-client).
