...
- Google Drive/Sheets accessible via service account amex-reconcile@hermes-499012.iam.gserviceaccount.com, key at ~/.gdrive-mcp/amex-service-account.json. Share Drive files/folders with this account for agent access. Also registered in Claude Code for cross-agent Drive access.
...
- Amex reconciliation: cron job reads credit card statements via Google Sheets service account (amex-reconcile@hermes-499012.iam.gserviceaccount.com, key at ~/.gdrive-mcp/amex-service-account.json).
- Projects under ~/projects/ auto-synced via repo-sync cron job.
