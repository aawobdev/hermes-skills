...
skills-compliant agent. Cloned to `~/projects/hermes-skills` (Linux/Mac) or
`%USERPROFILE%\projects\hermes-skills` (Windows) and referenced via Hermes config
...
```
scripts/                  Repo maintenance scripts (e.g. sync-memory.sh), not skill logic
shared/
├── hosts/<host>.md       One file per machine, curated from that host's local Hermes memory
└── learnings.md          AUTO-GENERATED by scripts/sync-memory.sh — concatenates
                          shared/hosts/*.md deterministically. Do not edit by hand.
```
...
- `blueprint-orchestration/references/cross-machine-memory-sync.md` — How `shared/hosts/*.md` and `shared/learnings.md` sync across machines
- `role-devops/references/docker-postgres-setup.md` — Docker + Postgres setup gotchas for the DevOps role
...
- **Role skills** (`role-*`) are system prompts. They have full Hermes SKILL.md frontmatter (name, description, metadata incl. `hermes.tags`) and load like any other skill when this repo is in `skills.external_dirs` — e.g. `/skill role-architect`. They're equally usable paste-and-relay: copy the SKILL.md body into a session with any model (including ones without Hermes/skill support) plus your project description. Either way they contain no tool instructions, just the role's persona, rules, and output format.
